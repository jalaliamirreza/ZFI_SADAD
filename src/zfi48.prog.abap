...
"     WHEN 'KBETR'.
 "      PERFORM set_catalog_text USING 'amount' CHANGING fieldcatalog.
  ...
  "   WHEN 'KSCHL'.
 "      PERFORM set_catalog_text USING 'condition type' CHANGING fieldcatalog.

 "    WHEN 'VTEXT'.
   "    PERFORM set_catalog_text USING 'description' CHANGING fieldcatalog.
...
" LEFT JOIN prcd_elements    AS prc10  ON  prc10~knumv = vbrk~knumv AND
 ...
 join t685t AS t  on   t~kschl = prc9~kschl
