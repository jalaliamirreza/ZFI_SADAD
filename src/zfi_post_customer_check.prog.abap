*&---------------------------------------------------------------------*
*& Report ZFI_POST_CUSTOMER_CHECK
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zfi_post_customer_check.
...
DATA: gv_it_output TYPE TABLE OF zfi_post_customer_check,
      gv_wa_output LIKE LINE OF  gv_it_output,
      gv_it_log    TYPE TABLE OF zfi_post_customer_check_log,
...
PARAMETERS: p_bukrs TYPE payr-zbukr NO-DISPLAY,
            ...
            p_blart TYPE bkpf-blart OBLIGATORY DEFAULT 'Z2',
...
PARAMETERS: p_portf TYPE bsed-portf OBLIGATORY,
            p_hkont TYPE bseg-hkont NO-DISPLAY,
...
FORM build_fieldcatalog .
  ...
  CALL FUNCTION 'REUSE_ALV_FIELDCATALOG_MERGE'
    EXPORTING
      i_structure_name = 'ZFI_POST_CUSTOMER_CHECK'
    CHANGING
  ...
  LOOP AT fieldcatalog.
    ...
    CASE fieldcatalog-fieldname.
      WHEN 'AMOUNT'.
        PERFORM set_catalog_text USING 'مبلغ چک' CHANGING fieldcatalog.
        "fieldcatalog-edit          = 'X'.
      ...
      WHEN 'ZFBDT' OR 'XREF1' OR 'XREF2' OR 'XREF3'.
        fieldcatalog-edit          = 'X'.
...
*      <--P_LV_GJAHR  text
*&---------------------------------------------------------------------*
FORM post_with_clear USING    lv_data      TYPE      zfi_post_customer_check
                     CHANGING lv_subrc     TYPE      sy-subrc
                              lv_belnr     TYPE      bkpf-belnr
                              lv_gjahr     TYPE      bkpf-gjahr.
  ...
  CLEAR lv_count.
  lv_count = lv_count + 1.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'RF05A-NEWKO' gv_wa_output-kunnr.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'RF05A-NEWBS' '09'.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'RF05A-NEWUM' 'W'.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'RF05A-PORTF' p_portf.

  WRITE gv_wa_output-amount TO lv_amount CURRENCY gv_wa_output-waers.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'BSEG-WRBTR'  lv_amount.
  PERFORM convert_date_to_ext USING gv_wa_output-zfbdt CHANGING lv_datet.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'BSEG-ZFBDT'  lv_datet.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'BSED-BOENO'  gv_wa_output-xref1.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'BSED-BANK'   gv_wa_output-xref3.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'BSED-ACCOU'  gv_wa_output-xref2.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'BSEG-XREF1'  gv_wa_output-xref1.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'BSEG-XREF2'  gv_wa_output-xref2.
  PERFORM fill_ftpost TABLES lv_it_ftpost USING 'P' lv_count 'BSEG-XREF3'  gv_wa_output-xref3.
...
FORM call_post .
  ...
  LOOP AT gv_it_output INTO gv_wa_output WHERE select = 'X' AND amount > 0 AND XREF1 <> ''.
    PERFORM post_with_clear USING gv_wa_output CHANGING lv_subrc lv_belnr lv_gjahr.
  ENDLOOP.
...
FORM display_log .
  ...
  CALL FUNCTION 'REUSE_ALV_FIELDCATALOG_MERGE'
    EXPORTING
      i_structure_name = 'ZFI_POST_CUSTOMER_CHECK_LOG'
    CHANGING
