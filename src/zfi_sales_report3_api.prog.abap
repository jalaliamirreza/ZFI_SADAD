...
FORM create_api_data_and_display.
  ...
  LOOP AT gv_it_output INTO DATA(ls_out_put).
    CLEAR ls_moadian1.
    ls_moadian1-internalserialnumber =  ls_out_put-xblnr. "شماره صورتحساب
    ls_moadian1-issuingdatetime = ls_out_put-fkdat. "تاریخ صورتحساب
    ls_moadian1-stuffServiceId = ls_out_put-shenaseh. " شناسه کالا / خدمات
    ls_moadian1-stuffServiceDescription = ls_out_put-arktx. " شرح کالا/خدمت
    ls_moadian1-SETTLEMENTMETHOD = 1.  " نقدي
    ls_moadian1-asn_num = ls_out_put-asn_num.
    ls_moadian1-asn_dat = ls_out_put-asn_dat.
    ls_moadian1-netweight = ls_out_put-ntgew.
    ls_moadian1-WEIGHTUOM = ls_out_put-GEWEI.
**-------------------------- " نوع صورتحساب
    IF ls_out_put-vkgrp = '170'.
      ls_moadian1-invoiceType  = '2'.
    ELSE.
      ls_moadian1-invoiceType  = '1'.
    ENDIF.
**-------------------------- " الگوی صورتحساب
    IF ls_out_put-CMWAE = 'IRR'.
      ls_moadian1-invoicePattern  = '1'.
    ELSEIF ls_out_put-CMWAE = 'USD'.
      ls_moadian1-invoicePattern  = '2'.
    ENDIF.
...
**-------------------------- " اموضوع صورتحساب
    ...
    ELSEIF ls_out_put-fkart = 'ZF50'
      ...
      ls_moadian1-invoiceSubject = '4'.
    ENDIF.
*
**-------------------------- " نوع خریدار
    IF     ls_out_put-ag_org = ' '.
      ls_moadian1-typeOfBuyer = '1'.
    ELSE.
      ls_moadian1-typeOfBuyer = '2'.
    ENDIF.
**-------------------------- " شناسه ملی خریدار
    IF ls_out_put-ag_org = ''.
      ls_moadian1-buyerNationalIdentity = ls_out_put-ag_ir4.
    ELSE.
      ls_moadian1-buyerNationalIdentity = ls_out_put-ag_ir3.
    ENDIF.
    ls_moadian1-buyerEconomicNumber = ls_out_put-ag_ir0. "کد اقتصادی خریدار
    ls_moadian1-quantity = ls_out_put-fkimg. " مقدار / تعداد
    ls_moadian1-fee = ls_out_put-amount1. " مبلغ واحد
    ls_moadian1-discount = ls_out_put-amount5 * -1. " مبلغ تخفیف
*------------------------------------ نوع ارز
    IF ls_out_put-vkgrp EQ '200'.
      ls_moadian1-currencyType = '840'.
    ELSE.
      ls_moadian1-currencyType = '0'.
    ENDIF.
*    *------------------------------------ واحد اندازه گیری و سنجش
    CASE ls_out_put-vrkme.
      WHEN 'KG'.
        ls_moadian1-measurementUnit = '164'.
      WHEN 'ST'.
        ls_moadian1-measurementUnit = '1668'.
      WHEN 'L'.
        ls_moadian1-measurementUnit = '1637'.
      WHEN 'TAG'.
        ls_moadian1-measurementUnit = '16104'.
    ENDCASE.
    IF ls_out_put-ag_org IS INITIAL.  " نام خريدار
      CONCATENATE ls_out_put-ag_last ' ' ls_out_put-ag_first INTO ls_moadian1-buyerName SEPARATED BY ' '.
    ELSE.
    ENDIF.
...
FORM build_fieldcat_mod1.
  CALL FUNCTION 'REUSE_ALV_FIELDCATALOG_MERGE'
   ...
   loop at fieldcatalog1 ASSIGNING FIELD-SYMBOL(<fs_fcat>).
