...
*&
*&---------------------------------------------------------------------*
REPORT zfi_customer_payment_outbound1.
...
, vbak~vbeln as vvbeln, vbak~vkgrp , vbap~spart as vspart , vbak~vkorg, vbap~posnr , vbak~auart as vauart , m~mvgr5 as vmvgr5
     FROM bkpf
    INNER JOIN bseg ON bseg~bukrs = bkpf~bukrs AND bseg~belnr = bkpf~belnr AND bseg~gjahr = bkpf~gjahr
    INNER JOIN but000 ON but000~partner = bseg~kunnr
    INNER JOIN vbfa ON vbfa~vbelv = bseg~vbel2 AND bldat = @p_begda
    INNER JOIN vbak on vbak~vbeln = vbfa~vbelv
    INNER JOIN vbap on vbak~vbeln = vbap~vbeln

    LEFT OUTER JOIN tb002 ON tb002~bu_group = but000~bu_group AND spras = 'EN'
    LEFT OUTER JOIN t012 ON t012~hbkid = bseg~hbkid AND t012~bukrs =
    bseg~bukrs AND  t012~banks = 'IR'
    LEFT OUTER JOIN bnka ON bnka~bankl = t012~bankl AND  bnka~banks =
    'IR'
    LEFT OUTER JOIN likp ON likp~vbeln = vbfa~vbeln AND vbfa~vbtyp_n = 'J'
    ...
    WHERE bseg~kunnr IN @p_kunnr AND bkpf~bukrs = @p_bukrs AND budat  BETWEEN @lv_begda AND @lv_endda AND
    but000~bu_group IN @p_cgrp  AND "and auart in @p_auart and
    bkpf~stblg = '' AND ( blart = 'DA' OR blart = 'DZ' OR blart = 'Z2'  OR blart = 'OO' )
    AND bseg~koart = 'D'
    AND ( ( umskz = 'A' AND bseg~bschl = '19' ) OR ( bseg~bschl = '09' AND umskz = 'F' AND augbl = '' ) OR ( bseg~bschl = '09'
    AND umskz = 'W' ) OR ( bseg~bschl = '15' AND umskz = '' ) ) GROUP BY  bseg~h_waers,  bseg~h_blart,
        bseg~bschl , bseg~bukrs, bseg~augbl  , bseg~buzei , bseg~gjahr ,
