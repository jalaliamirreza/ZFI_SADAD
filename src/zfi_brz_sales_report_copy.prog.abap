...
*&
*&---------------------------------------------------------------------*
REPORT zfi_brz_sales_report_copy.
...
TYPES: BEGIN OF t_output,
         ...
         netvf05                   TYPE prcd_elements-kbetr,
         nettotal                  TYPE prcd_elements-kbetr,
       END OF t_output.
...
DATA: lt_customer LIKE STANDARD TABLE OF ls_customer WITH KEY kunrg,
      ...
      DATA: BEGIN OF ls_taxnum ,
        taxtnum TYPE  dfkkbptaxnum-taxnum,
...
START-OF-SELECTION.
  ...
  SELECT DISTINCT
     vbrp~ntgew , vbrp~brgew ,vbrk~buchk, vbrk~vbeln , vbrk~fkart,vbrp~werks , tvfkt~vtext , vbrk~fkdat , bkpf~xblnr , bkpf~belnr , vbrk~cmwae , vbrk~zterm , vbrk~kunrg , vbrk~spart , vbrk~kdgrp  , vbrp~posnr , vbrp~matnr , vbrp~vrkme , vbrp~arktx ,
     vbrp~aubel , vbrp~aupos , vbrp~vgbel , vbrp~vgpos , vbrp~fkimg , vbrp~spart AS sparti , a~kbetr , a~kwert , g~charg , t005u~bezei AS regiontxt, mara~matkl ,t023t~wgbez, mara~groes , vbrk~vbtyp, vbrk~fkart , vbrk~vkorg , vbrk~vtweg, vbak~erdat,
    vbak~kvgr1,vbak~kvgr2,vbak~kvgr3,vbak~kvgr4,vbak~kvgr5,vbrp~batch_ntgew,vbrp~batch_gewei,
...
SUM( DISTINCT c~kwert ) AS tax , d~kwert AS cleared_downpayment , SUM( DISTINCT e~kwert ) AS delivery_cost , SUM( DISTINCT f~kwert ) AS packaging_cost
, adrc~region,
     adrc~city1,
     adrc~post_code1,
     adrc~street,
     ...
     adr2~tel_number
      FROM vbrk INNER JOIN tvfkt  ON tvfkt~fkart = vbrk~fkart
              ...
              LEFT OUTER JOIN prcd_elements AS e ON e~knumv = vbrk~knumv AND  e~kposn = vbrp~posnr AND e~kstat = '' AND e~kschl LIKE 'ZH%' AND e~kinak = ''
              ...
              INNER JOIN t005u ON t005u~bland = vbrk~regio AND t005u~land1 = vbrk~land1 AND t005u~spras = 'E'
...
*LEFT JOIN tvv1t AS tvv1t1 ON vbak~kvgr1 = tvv1t1~kvgr1 AND tvv1t1~spras = 'E'
*LEFT JOIN tvv2t AS tvv2t2 ON vbak~kvgr2 = tvv2t2~kvgr2 AND tvv2t2~spras = 'E'
*LEFT JOIN tvv3t AS tvv3t3 ON vbak~kvgr3 = tvv3t3~kvgr3 AND tvv3t3~spras = 'E'
*LEFT JOIN tvv4t AS tvv4t4 ON vbak~kvgr4 = tvv4t4~kvgr4 AND tvv4t4~spras = 'E'
*LEFT JOIN tvv5t AS tvv5t5 ON vbak~kvgr5 = tvv5t5~kvgr5 AND tvv5t5~spras = 'E'
...
WHERE
              vbrk~fkdat IN @p_fkdat AND vbrk~kunrg IN @p_kunrg AND vbrk~vbeln IN @p_vbeln
               AND buchk IN @p_buchk
              AND a~koaid = 'B' AND a~kstat = '' AND a~kinak = ''
              AND c~kstat = '' AND ( c~kschl = 'ZVAT' OR c~kschl = 'ZVAD' ) AND tvfkt~spras = 'E'
              AND vbrk~fkart IN @p_fkart AND vbrk~spart IN @p_spart AND vbrk~vkorg IN @p_vkorg AND vbrk~regio IN @p_regio AND vbak~vkgrp IN @p_vkgrp AND vbrk~kdgrp IN @p_kdgrp
              AND vbrp~werks IN @p_werks
              AND vbrk~bukrs IN @p_bukrs
              AND mara~matkl IN @s_matkl
              AND vbap~mvgr5 IN @s_mvgr5
    GROUP BY  vbrp~ntgew , vbrp~brgew ,vbrk~buchk ,vbrk~vbeln , vbrk~fkart ,vbrp~werks, tvfkt~vtext , vbrk~fkdat ,
              bkpf~xblnr , bkpf~belnr , vbrp~posnr , vbrp~matnr , vbrp~arktx , vbrp~aubel , vbrp~aupos , vbrp~vgbel , vbrp~vgpos , vbrp~fkimg , vbrp~spart ,
               a~kbetr , a~kwert , d~kwert , vbrk~cmwae , vbrk~zterm , vbrp~vrkme , vbrk~kunrg, vbrk~spart , vbrk~kdgrp, g~charg, t005u~bezei
               , mara~matkl ,t023t~wgbez, mara~groes, vbrk~vbtyp, vbrk~fkart , vbrk~vkorg,vbrk~vtweg, vbak~erdat,
        vbak~kvgr1,vbak~kvgr2,vbak~kvgr3,vbak~kvgr4,vbak~kvgr5,vbrp~batch_ntgew,vbrp~batch_gewei,
*    tvv1t1~bezei,
*    tvv2t2~bezei,
*    tvv3t3~bezei,
...
*    tvm5t~bezei,
  ...
  SELECT kunnr AS kunrg ktokd rpmkr FROM kna1  INTO CORRESPONDING FIELDS OF TABLE lt_customer FOR ALL ENTRIES IN lt_output  WHERE kunnr = lt_output-kunrg AND kunnr IN p_kunrg.
  ...
  LOOP AT lt_customer INTO ls_customer.
    ...
    SELECT  taxnum taxtype FROM dfkkbptaxnum INTO CORRESPONDING FIELDS OF TABLE lt_taxnum WHERE taxtype IN ( 'IR0' ,'IR1' ,'IR2' ,'IR3' ,'IR4' ,'IR5'  ) AND partner = ls_customer-kunrg.


    LOOP AT lt_taxnum.
      CASE lt_taxnum-taxtype.
        WHEN 'IR0'.
          ls_customer-ecco_no = lt_taxnum-taxtnum.
        WHEN 'IR1'.
          ls_customer-register_no = lt_taxnum-taxtnum.
        WHEN 'IR2'.
          ls_customer-birth_no = lt_taxnum-taxtnum.
        WHEN 'IR3'.
          ls_customer-national_id = lt_taxnum-taxtnum.
        WHEN 'IR4'.
          ls_customer-nationalcode = lt_taxnum-taxtnum.
        WHEN 'IR5'.
          ls_customer-passport_no = lt_taxnum-taxtnum.
  ...
  LOOP AT lt_output INTO ls_output.
    ...
    IF sy-subrc IS INITIAL.
      ...
      SELECT SINGLE bezei INTO ls_output-partner_bezei FROM t005u
        WHERE spras = 'EN' AND land1 = 'IR' AND bland = ls_customer-region.
...
*    SELECT SINGLE taxnum INTO ls_output-t1 FROM dfkkbptaxnum WHERE partner = ls_output-kunrg AND taxtype = 'IR0'.
*    SELECT SINGLE taxnum INTO ls_output-t2 FROM dfkkbptaxnum WHERE partner = ls_output-kunrg AND taxtype = 'IR1'.
*    SELECT SINGLE taxnum INTO ls_output-t3 FROM dfkkbptaxnum WHERE partner = ls_output-kunrg AND taxtype = 'IR2'.
*    SELECT SINGLE taxnum INTO ls_output-t4 FROM dfkkbptaxnum WHERE partner = ls_output-kunrg AND taxtype = 'IR3'.
*    SELECT SINGLE taxnum INTO ls_output-t5 FROM dfkkbptaxnum WHERE partner = ls_output-kunrg AND taxtype = 'IR4'.
*    SELECT SINGLE taxnum INTO ls_output-t6 FROM dfkkbptaxnum WHERE partner = ls_output-kunrg AND taxtype = 'IR5'.
    ...
    IF ls_output-mvgr5 IS INITIAL.
      SELECT SINGLE mvke~mvgr5 tvm5t~bezei AS mvgr5t INTO CORRESPONDING FIELDS OF ls_output
        FROM mvke
        LEFT JOIN tvm5t ON mvke~mvgr5 = tvm5t~mvgr5 AND tvm5t~spras = 'E'
...
*       ex_datfmused =
    ...
    IF sy-subrc IS INITIAL.
      ...
      ls_output-total_netamount = ls_output1-kwert + ls_output1-discount." + ls_output1-tax.
      ls_output-total_cleared_downpayment = ls_output1-cleared_downpayment.
      ls_output-payable_amount = ls_output-total_netamount + ls_output1-tax + ls_output-total_packaging_cost + ls_output-total_delivery_cost - ls_output-total_cleared_downpayment.
      ls_output-netamount = ls_output-kwert + ls_output-discount." + ls_output-tax.
    ENDIF.
...
*         OTHERS = 2
    ...
    IF ls_output-fkart = 'ZT01' OR ls_output-fkart = 'ZT00'  OR ls_output-fkart = 'ZB01' OR ls_output-fkart = 'ZB00' OR ls_output-fkart = 'YT00' OR ls_output-fkart = 'YT01'.
      ls_output-fkimg = 0.
      ls_output-ausp1 = 0.
      ls_output-ntgew = 0.
      ls_output-brgew = 0.
    ...
    IF ( ls_output-vbtyp = 'O' AND ls_output-fkart <> 'ZT01' AND ls_output-fkart <> 'ZT00') OR ( ls_output-vbtyp = 'N' AND ls_output-fkart <> 'ZT01' AND ls_output-fkart <> 'ZT00').
      ...
      ls_output-total_cleared_downpayment =	ls_output-total_cleared_downpayment * -1.
      ls_output-total_delivery_cost =	ls_output-total_delivery_cost * -1.
      ls_output-total_packaging_cost =  ls_output-total_packaging_cost * -1.
      ls_output-total_netamount =	ls_output-total_netamount * -1.
      ls_output-payable_amount =  ls_output-payable_amount * -1.
    ELSEIF  ls_output-fkart = 'ZT01' OR ls_output-fkart = 'ZT00'." OR ls_output-fkart = 'YT01' OR ls_output-fkart = 'YB00' .
      ls_output-kwert	= ls_output-kwert * -1.
      ls_output-kbetr =	ls_output-kbetr * -1.
      ls_output-discount =  ls_output-discount * -1.
      ls_output-tax =	ls_output-tax * -1.
    ...
    SELECT adrc~post_code1 UP TO 1 ROWS FROM but020
     ...
     WHERE partner = ls_output-vbpa_kunnr AND adrc~langu = p_lang  ORDER BY addr_valid_from DESCENDING.
    ENDSELECT.
